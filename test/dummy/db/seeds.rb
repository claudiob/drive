...
agents = ['ada@example.com', 'grace@example.com'].map do |email|
  Agent.find_or_create_by! email:
end
...
locations = ZIP.order(:code).limit(4).each_with_index.map do |zip, index|
  Location.find_or_create_by! zip:, street: "#{100 + index} Main Street" do |location|
    location.city = zip.city
    location.agent = agents[index]
  end
end
